...
// import { buildTailwindAndSave } from "./tailwind.js";
    ...
    async function buildTailwindFactory(): Promise<() => Promise<void>> {
        ...
        try {
            ...
            try {
                ...
                inputCSS = new TextDecoder("utf-8").decode( Deno.readFileSync("src/input.css") );
    
            }catch{}

            const buildTailwindAndSave = tailwindConfig ? (await lazyLoad("./tailwind.js")).buildTailwindAndSave : null;

            const buildFn = async () => {
                if(buildTailwindAndSave) 
                    await buildTailwindAndSave(tailwindConfig, inputCSS, "public/css/output.css");
            }
    ...
    async function defaultHandler (request: Request): Promise<Response> {
        ...
        if (file) {
            ...
            try {
                ...
                const state = await getAppState({ ...(config?.model ?? {}), request: extendedRequest });
                ...
                body = 
                    /*html*/`<link id=tailwind" rel="stylesheet" href="/css/output.css?hash=">` +
                    ...
                    <div id="scripts">
                        ...
                        <script src="${config?.clientScriptURL || "https://jsr.io/@luxemburgo/journey/0.15.3/src/client/index.js"}" type="module"></script>
                        <script>
                            window.journey = {
                                router: {
        ...
        if (file) {
            ...
            response.headers.set(
                "Cache-Control",
                (typeof config?.cacheControl == "function" ? config.cacheControl(extendedRequest.url) : config?.cacheControl) || 
                (extendedRequest.url.href.match(/\.js$|\?$/g) ? "must-revalidate, no-store, no-cache, private" : "public, max-age=360000")
            );
